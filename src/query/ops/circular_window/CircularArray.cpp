 ...
 *
 * @brief Circular window array implementation
 *
...
static log4cxx::LoggerPtr circularwindowLogger(log4cxx::Logger::getLogger("scidb.query.ops.window"));
...
namespace scidb
{
     ...
     */
    int MaterializedCWindowChunkIterator::getMode() const
    {
     ...
     */
    void MaterializedCWindowChunkIterator::calculateNextValue()
    {
        if(_currPos > _numToCalculate)
        {
     ...
     */
    Value const& MaterializedCWindowChunkIterator::getItem()
    {
        if (end())
     ...
     */
    Coordinates const& MaterializedCWindowChunkIterator::getPosition()
    {
        if (end())
            throw USER_EXCEPTION(SCIDB_SE_EXECUTION, SCIDB_LE_NO_CURRENT_ELEMENT);
     ...
     */
    bool MaterializedCWindowChunkIterator::setPosition(Coordinates const& pos)
    {
     ...
     */
    bool MaterializedCWindowChunkIterator::isEmpty() const
    {
        return false;
    }

    void MaterializedCWindowChunkIterator::stepToNextValidValue()
    {
        while (!end()) {
            calculateNextValue();
            if ((_iterationMode & IGNORE_NULL_VALUES && _nextValue.isNull()) ||
     ...
     */
    void MaterializedCWindowChunkIterator::reset()
    {
     ...
     */
    void MaterializedCWindowChunkIterator::operator ++()
    {
        ++_iter;
        stepToNextValidValue();
    }
     ...
     */
    bool MaterializedCWindowChunkIterator::end()
    {
     ...
     */
    ConstChunk const& MaterializedCWindowChunkIterator::getChunk()
    {
        return _chunk;
    }
    ...
    //**********************************************************************************//
    //Window Circular Chunk
    WindowCircularChunk::WindowCircularChunk(WindowCircularArray const& arr, AttributeID attr)
            : _array(arr),
              _arrayIterator(NULL),
     ...
     */
    Array const& WindowCircularChunk::getArray() const
    {
        return _array;
    }

    inline uint64_t WindowCircularChunk::getStep() const
    {
     ...
     */
    const ArrayDesc& WindowCircularChunk::getArrayDesc() const
    {
        return _array._desc;
    }
     ...
     */
    const AttributeDesc& WindowCircularChunk::getAttributeDesc() const
    {
     ...
     */
    Coordinates const& WindowCircularChunk::getFirstPosition(bool withOverlap) const
    {
        return _firstPos;
    }
     ...
     */
    Coordinates const& WindowCircularChunk::getLastPosition(bool withOverlap) const
    {
        return _lastPos;
    }
     ...
     */
    std::shared_ptr<ConstChunkIterator> WindowCircularChunk::getConstIterator(int iterationMode) const
    {
        ...
        return std::shared_ptr<ConstChunkIterator>(new MaterializedCWindowChunkIterator(*_arrayIterator, *this, iterationMode));
     ...
     */
    int WindowCircularChunk::getCompressionMethod() const
    {
     ...
     */
    inline uint64_t WindowCircularChunk::coord2pos(Coordinates const& coord) const
    {
     ...
     * Private function to compute the position in the chunk from coordinates
     */
    inline void WindowCircularChunk::pos2coord(uint64_t pos, Coordinates& coord) const
    {
        SCIDB_ASSERT(_materialized);
     ...
     */
    inline bool WindowCircularChunk::valueIsNeededForAggregate ( const Value & val, const ConstChunk & inputChunk ) const
    {
     ...
     */
    void WindowCircularChunk::materialize()
    {
        _materialized = true;
        _stateMap.clear();
        _inputMap.clear();
     ...
     */
    void WindowCircularChunk::setPosition(WindowCircularArrayIterator const* iterator, Coordinates const& pos)
    {
        ...
        if (_array._desc.getEmptyBitmapAttribute())
        {
            ...
            //   function.
            //
            if (_arrayIterator->getMethod() == WindowCircularArray::MATERIALIZE)
            {
                materialize();
            } else if (_arrayIterator->getMethod() != WindowCircularArray::PROBE)
            {
                //
                ...
                if ( materializedChunkSize <= maxMaterializedChunkSize )
                {
                    ...
                    LOG4CXX_TRACE ( circularwindowLogger,
                    ...
                    LOG4CXX_TRACE ( circularwindowLogger, "\t NOT MATERIALIZING ");
                }
            }
        }
    }
    ...
    WindowCircularArrayIterator::WindowCircularArrayIterator(WindowCircularArray const& arr, AttributeID attrID, AttributeID input,  string const& method)
            : array(arr),
     ...
     *  @see ConstIterator::operator ++()
     */
    void WindowCircularArrayIterator::operator ++()
    {
        if (!hasCurrent)
            throw USER_EXCEPTION(SCIDB_SE_EXECUTION, SCIDB_LE_NO_CURRENT_ELEMENT);
     ...
     *  @see ConstIterator::end()
     */
    bool WindowCircularArrayIterator::end()
    {
        return !hasCurrent;
    }
     ...
     *  @see ConstIterator::getPosition()
     */
    Coordinates const& WindowCircularArrayIterator::getPosition()
    {
        if (!hasCurrent)
            throw USER_EXCEPTION(SCIDB_SE_EXECUTION, SCIDB_LE_NO_CURRENT_ELEMENT);
        return currPos;
     ...
     *  @see ConstIterator::setPosition()
     */
    bool WindowCircularArrayIterator::setPosition(Coordinates const& pos)
    {
        chunkInitialized = false;
     ...
     *  @see ConstIterator::reset()
     */
    void WindowCircularArrayIterator::reset()
    {
        chunkInitialized = false;
        iterator->reset();
        hasCurrent = !iterator->end();
     ...
     */
    ConstChunk const& WindowCircularArrayIterator::getChunk()
    {
        if (!chunkInitialized)
        {
     ...
     */
    ArrayDesc const& WindowCircularArray::getArrayDesc() const
    {
        return _desc;
    }
     ...
     */
    std::shared_ptr<ConstArrayIterator> WindowCircularArray::getConstIterator(AttributeID attr) const
    {
        if (_desc.getEmptyBitmapAttribute() && attr == _desc.getEmptyBitmapAttribute()->getId())
        {
            return std::shared_ptr<ConstArrayIterator>(new WindowCircularArrayIterator(*this, attr, _inputArray->getArrayDesc().getEmptyBitmapAttribute()->getId(), _method));
        }

        return std::shared_ptr<ConstArrayIterator>(new WindowCircularArrayIterator(*this, attr, _inputAttrIDs[attr], _method ));
    }
}
