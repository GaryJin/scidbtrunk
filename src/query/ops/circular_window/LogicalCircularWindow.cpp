...
/**
 * @brief The operator: circular_window().
 *
 * @par Synopsis:
 *   circular_window( srcArray {, leftEdge, rightEdge}+ {, AGGREGATE_CALL}+ [, METHOD ] )
 ...
 *
 */
    class LogicalCircularWindow: public LogicalOperator
    {
    public:

        LogicalCircularWindow(const std::string& logicalName, const std::string& alias):
         ...
         *
         *   The output array of the circular_window(...) operator is the same
         *   size and shape as the input, and has a set of attributes the same
         *   size and type as the aggregates.
         *
                ...
                switch ( param->getParamType() ) {
                    ...
                    case PARAM_LOGICAL_EXPRESSION:
                    {
                                    ...
                                    ().typeId()))
                        {
                            ...
                            if (!((s == WindowCircularArray::PROBE) || (s == WindowCircularArray::MATERIALIZE )))
                            {
                                stringstream ss;
                                ss << WindowCircularArray::PROBE << " or " << WindowCircularArray::MATERIALIZE;
    ...
    DECLARE_LOGICAL_OPERATOR_FACTORY(LogicalCircularWindow, "circular_window")
